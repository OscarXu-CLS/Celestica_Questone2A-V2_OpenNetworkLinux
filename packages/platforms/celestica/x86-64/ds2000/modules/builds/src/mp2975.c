				 ...
				 int reg)
{
	...
	case PMBUS_READ_VOUT:
		...
		if (data->vout_format[page] == vid)
			...
			ret = mp2975_vid2direct(info->vrm_version, ret);  //to suit kernel v5.4.40
		break;
		    ...
		    u32 imvp_bit, u32 vr_bit)
{
	...
	if (ret & imvp_bit) {
		...
		info->vrm_version = imvp9; //to suit kernel v5.4.40
		data->vid_step[page] = MP2975_PROT_DEV_OV_OFF;
	} else if (ret & vr_bit) {
		//info->vrm_version[page] = vr12;
		info->vrm_version = vr12; //to suit kernel v5.4.40
		data->vid_step[page] = MP2975_PROT_DEV_OV_ON;
	} else {
		//info->vrm_version[page] = vr13;
		info->vrm_version = vr13; //to suit kernel v5.4.40
		data->vid_step[page] = MP2975_PROT_DEV_OV_OFF;
	}
		    ...
		    struct pmbus_driver_info *info, int page)
{
	...
	//					 GENMASK(8, 0));
	data->vout_max[page] = mp2975_vid2direct(info->vrm_version, ret &
						 GENMASK(8, 0));  //to suit kernel v5.4.40
	return 0;
}
