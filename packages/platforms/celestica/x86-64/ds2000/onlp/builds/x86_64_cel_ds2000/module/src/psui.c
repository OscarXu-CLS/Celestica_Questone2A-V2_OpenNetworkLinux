...
int psu1_src = 0, psu2_src = 0;

static onlp_psu_info_t psu_info[] =
    {
        {},
        {
            {ONLP_PSU_ID_CREATE(PSU1_ID), "PWR1", 0},
            "",
            "",
            0,
        ...
        {
            {ONLP_PSU_ID_CREATE(PSU2_ID), "PWR2", 0},
            "",
            "",
            0,
            ONLP_PSU_CAPS_AC | ONLP_PSU_CAPS_VIN | ONLP_PSU_CAPS_VOUT | ONLP_PSU_CAPS_IIN | ONLP_PSU_CAPS_IOUT | ONLP_PSU_CAPS_PIN | ONLP_PSU_CAPS_POUT,
...
int onlp_psui_info_get(onlp_oid_t id, onlp_psu_info_t *info_p)
{
    ...
    *info_p = psu_info[psu_id];
    ...
    pwrok_status = (psu_status >> psu_mapper[psu_id].bit_pwrok_sta) & 0x01;
    ...
    if(psu_id == 1){
        psu_offset = 1;
        psu1_src = check_psu_bom(psu_id, info_p->model);
    }else if(psu_id == 2){
        psu_offset = 3;
        psu2_src = check_psu_bom(psu_id, info_p->model);
    }
