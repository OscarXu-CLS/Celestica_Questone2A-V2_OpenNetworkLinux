...
#include "x86_64_cel_ds2000_int.h"
...
{
    return "x86-64-cel-ds2000-r0";
}

int onlp_sysi_init(void)
{
    pthread_mutex_init(&lock, NULL);
    bmc_status = read_register(CARD_PRESENT_STATE_REG, SYS_CPLD_PATH) & 0x01;
...
int onlp_sysi_platform_info_get(onlp_platform_info_t *pi)
{
    ...
    pi->cpld_versions = aim_fstrdup("Baseboard CPLD version is %s", r_data);
    ...
    pi->other_versions = aim_fstrdup("FPGA version is %s", r_data);
